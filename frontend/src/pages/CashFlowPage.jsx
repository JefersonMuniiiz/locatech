...
const EQUIPMENT_TYPES = {
  SCAFFOLD: 'Andaime', PROP: 'Escora', CONCRETE_MIXER: 'Betoneira',
  MACHINE: 'Máquina', TOOL: 'Ferramenta', OTHER: 'Outros',
}
...
export default function CashFlowPage() {
  ...
  const periodo = {
    receitas: data?.totalReceitas || 0,
    despesas: data?.totalDespesas || 0,
    saldo: data?.saldo || 0,
    pendente: data?.totalPendente || 0,
    pctReceitas: data?.pctReceitas,
    pctDespesas: data?.pctDespesas,
  }

  // Resumo anual
  const ano = data?.anoSummary || { totalReceitas: 0, totalDespesas: 0, saldo: 0, totalPendente: 0 }
    ...
    <div className="space-y-5">
        ...
        <div className="card p-5 border-l-4 border-green-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-green-600">{isLoading ? '—' : formatCurrency(periodo.receitas)}</p>
              {periodo.pctReceitas && <p className={`text-xs ${Number(periodo.pctReceitas) >= 0 ? 'text-green-500' : 'text-red-500'}`}>{Number(periodo.pctReceitas) >= 0 ? '↑' : '↓'} {Math.abs(periodo.pctReceitas)}% vs mês anterior</p>}
            </div>
          </div>
        </div>
        ...
        <div className="card p-5 border-l-4 border-red-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-red-600">{isLoading ? '—' : formatCurrency(periodo.despesas)}</p>
              {periodo.pctDespesas && <p className={`text-xs ${Number(periodo.pctDespesas) <= 0 ? 'text-green-500' : 'text-red-500'}`}>{Number(periodo.pctDespesas) >= 0 ? '↑' : '↓'} {Math.abs(periodo.pctDespesas)}% vs mês anterior</p>}
            </div>
          </div>
        </div>

        <div className={`card p-5 border-l-4 ${periodo.saldo >= 0 ? 'border-blue-500' : 'border-orange-500'}`}>
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${periodo.saldo >= 0 ? 'bg-blue-50' : 'bg-orange-50'}`}>
              <DollarSign size={20} className={periodo.saldo >= 0 ? 'text-blue-600' : 'text-orange-600'} />
            </div>
            <div>
              <p className="text-xs text-slate-500">Saldo do Período</p>
              <p className={`text-xl font-bold ${periodo.saldo >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>{isLoading ? '—' : formatCurrency(periodo.saldo)}</p>
        ...
        <div className="card p-5 border-l-4 border-yellow-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-yellow-600">{isLoading ? '—' : formatCurrency(periodo.pendente)}</p>
      ...
      <div className="card p-5">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data?.chartData || []} barGap={2}>
            ...
            <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={v => `R$${Math.abs(v) >= 1000 ? (v/1000).toFixed(0)+'k' : v}`} />
      ...
      <div className="card overflow-hidden">
        <div className="flex border-b border-slate-200">
          {['overview','expenses','revenues'].map((tab, i) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${activeTab === tab ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
              {['Resumo do Ano','Despesas do Período','Receitas do Período'][i]}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <div className="p-5">
            <p className="text-xs text-slate-400 mb-4">Totais acumulados em {now.getFullYear()}</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-center">
              <div className="p-4 bg-green-50 rounded-lg">
                <p className="text-xs text-slate-500 mb-1">Total Receitas</p>
                <p className="text-lg font-bold text-green-600">{formatCurrency(ano.totalReceitas)}</p>
              </div>
              <div className="p-4 bg-red-50 rounded-lg">
                <p className="text-xs text-slate-500 mb-1">Total Despesas</p>
                <p className="text-lg font-bold text-red-600">{formatCurrency(ano.totalDespesas)}</p>
              </div>
              <div className="p-4 bg-yellow-50 rounded-lg">
                <p className="text-xs text-slate-500 mb-1">A Receber</p>
                <p className="text-lg font-bold text-yellow-600">{formatCurrency(ano.totalPendente)}</p>
              </div>
              <div className={`p-4 rounded-lg ${ano.saldo >= 0 ? 'bg-blue-50' : 'bg-orange-50'}`}>
                <p className="text-xs text-slate-500 mb-1">Saldo do Ano</p>
                <p className={`text-lg font-bold ${ano.saldo >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>{formatCurrency(ano.saldo)}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className="input" type="number" step="0.01" min="0.01" required
                value={form.amount} onChange={e => set('amount', e.target.value)} onFocus={e => e.target.select()} />
            </div>
            <div>
