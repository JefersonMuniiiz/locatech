...
export default function EditRentalPage() {
  ...
  useEffect(() => {
    ...
    setItems(rental.items?.map(i => ({ equipmentId: i.equipmentId, quantity: i.quantity })) || [])
    setDiscountPercent(rental.discountPercent || 0)
  }, [rental])
  ...
  const update = useMutation({
    mutationFn: d => api.put(`/rentals/${id}`, d),
    onSuccess: () => {
  ...
  const handleSubmit = (e) => {
    ...
    update.mutate({ ...form, items, discount })
  }

  if (rentalLoading) return <div className="card p-8 text-center text-slate-400">Carregando...</div>
  ...
  return (
    <div className="max-w-3xl space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Editar Locação</h1>
          {rental?.contractNumber > 0 && <p className="text-slate-500 text-sm">Contrato #{String(rental.contractNumber).padStart(4,'0')}</p>}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="card p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="btn-secondary text-sm" onClick={addItem}><Plus size={14} /> Adicionar</button>
          </div>
          {items.length === 0 && (
              ...
              <div key={i} className="grid grid-cols-12 gap-3 items-end p-3 bg-slate-50 rounded-lg">
                <div className="col-span-6">
                  ...
                  <select className="input" value={item.equipmentId} onChange={e => setItem(i, 'equipmentId', e.target.value)}>
                    ...
                    {equipments?.map(e => <option key={e.id} value={e.id}>{e.name} (disp: {e.availableQuantity + (item.equipmentId === e.id ? item.quantity : 0)})</option>)}
                  </select>
                </div>
                <div className="col-span-2">
                ...
                <div className="col-span-3 text-sm">
                  ...
                  <div className="py-2 px-3 bg-white border border-slate-200 rounded-lg font-semibold text-slate-700">{formatCurrency(itemTotal)}</div>
                </div>
        ...
        <div className="card p-5 bg-slate-800 text-white">
          ...
          <div className="grid grid-cols-3 gap-4 text-sm mb-4">
            <div><p className="text-slate-400">Período</p><p className="font-bold text-lg">{totalDays} dias</p></div>
            <div><p className="text-slate-400">Itens</p><p className="font-bold text-lg">{items.length}</p></div>
            <div><p className="text-slate-400">Subtotal</p><p className="font-bold text-lg">{formatCurrency(subtotal)}</p></div>
          </div>
          <div className="flex rounded-lg overflow-hidden border border-slate-600 mb-3">
            <button type="button" onClick={() => { setDiscountMode('percent'); setCustomValue('') }}
            ...
            <button type="button" onClick={() => { setDiscountMode('value'); setDiscountPercent(0) }}
          ...
          <div className="flex items-center gap-3 p-3 bg-slate-700 rounded-lg mb-4">
            ...
            {discountMode === 'percent' ? (
              ...
              <>
                ...
                <input type="number" min="0" step="0.01" value={customValue}
                  onChange={e => setCustomValue(e.target.value)} onFocus={e => e.target.select()}
          ...
          <button type="submit" className="btn-primary w-full justify-center py-3 bg-blue-500 hover:bg-blue-400" disabled={update.isPending}>
            {update.isPending ? <Loader2 size={18} className="animate-spin" /> : 'Salvar Alterações'}
          </button>
        </div>
      </form>
    </div>
